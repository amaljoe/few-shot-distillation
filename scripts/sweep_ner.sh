#!/usr/bin/env bash
# NER lambda sweep — GPUs 2,3 — runs λ=0.05, 0.1, 0.2 sequentially
# Run in its own tmux window alongside sweep_pos.sh (GPUs 0,1)
...
MAX_STEPS=400
TASK=ner
TP=2   # eval stays on GPUs 2,3 so POS window can run simultaneously
...
log "=== NER SWEEP COMPLETE — see lambda_sweep_results.json ==="
