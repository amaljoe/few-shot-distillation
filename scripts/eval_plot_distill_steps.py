"""
Evaluate all 5 distillation-duration checkpoints on GSM8K (zero-shot)
and produce one accuracy-vs-distill-steps plot.

Reads LoRA adapters from:
  experiments/distill_steps/steps_{N}/final/
  for N in [0, 4, 16, 64, 200]

Writes:
  experiments/distill_steps/eval_results.json
  experiments/distill_steps/distill_steps_accuracy.png

Usage (no GPU flag needed — vLLM auto-detects):
...
BASE_DIR       = Path("experiments/distill_steps")
N_SAMPLES      = 1319   # full GSM8K test set
SEED           = 42
MAX_NEW_TOKENS = 512
MAX_MODEL_LEN  = 2048
TP_SIZE        = 4
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f" Distillation steps ablation — GSM8K zero-shot eval")
    ...
    if not available:
        print("No adapters found. Run distill_steps_experiment.py first.")
        return

    print(f"Evaluating conditions: {available}\n")

    # ── vLLM — load base model once, swap LoRA adapters per condition ──────────
    ...
    for i, n_distill in enumerate(available):
        ...
        lora_req = LoRARequest(f"steps_{n_distill}", i + 1, adapter_path)

        label = "SFT only" if n_distill == 0 else f"{n_distill}-step distill"
    ...
    # Use evenly-spaced positions with custom labels (values span 0–200 non-linearly)
    ...
    ax.plot(x_pos, y_vals, "o-", color="#2196F3", linewidth=2.5, markersize=9,
            markerfacecolor="white", markeredgewidth=2.5, markeredgecolor="#2196F3")
    ...
    ax.set_xlabel("Number of distillation steps (out of 200 total)", fontsize=11)
    ax.set_ylabel("GSM8K zero-shot accuracy (%)", fontsize=11)
    ax.set_title(
        "How many distillation steps are needed?\n(Qwen3-1.7B, LoRA, 200 total steps)",
    ...
    plt.tight_layout()
    out_png = BASE_DIR / "distill_steps_accuracy.png"
