"""
CommonsenseQA dataset loader and prompt formatter.

Dataset: tau/commonsense_qa
Fields: question (str), choices (dict with label/text lists), answerKey (A-E)

The test split has no labels; we redirect it to "validation".
...
def load_commonsenseqa(split: str = "train"):
    """Load CommonsenseQA from HuggingFace datasets."""
    actual = "validation" if split == "test" else split
    return load_dataset("tau/commonsense_qa", split=actual)


def format_choices(choices: dict) -> str:
    """Format choices dict into (A) text\\n(B) text... string."""
    return "\n".join(f"({l}) {t}" for l, t in zip(choices["label"], choices["text"]))


def get_answer_text(example: dict) -> str:
    """Return the canonical answer string for an example."""
    return f"The answer is ({example['answerKey']})."


def get_ground_truth(example: dict) -> str:
    """Return the ground truth answer letter (A-E)."""
...
def build_fewshot_messages(fewshot_examples: list[dict], query: dict) -> list[dict]:
    ...
    for ex in fewshot_examples:
        ...
        messages.append({"role": "assistant", "content": get_answer_text(ex)})
    choices_str = format_choices(query["choices"])
...
class CSQADistillDataset(Dataset):
    """
    Dataset that returns teacher + student inputs for each CommonsenseQA example.
    ...
    teacher_include_answer (bool): when True the teacher sequence includes the
        ...
        sequence. Required for online distillation (V1) where alignment is done
        on answer-token positions.
    """
    ...
    def __init__(
        ...
        num_fewshot: int = 5,
        max_seq_len_teacher: int = 1024,
        max_seq_len_student: int = 256,
        seed: int = 42,
        teacher_include_answer: bool = False,
        shuffle_fewshot_answers: bool = False,  # accepted, ignored for CSQA
    ):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.teacher_include_answer:
            teacher_messages_with_ans = teacher_messages + [
                {"role": "assistant", "content": get_answer_text(example)}
            ]
        ...
        # --- Labels: mask prompt tokens with -100, supervise on answer ---
